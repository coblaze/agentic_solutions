...
class GAutomationProcessor:
    ...
    def calculate_metrics(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        accuracy = (pass_count / total_count) if total_count > 0 else 0
    ...
    def save_results_with_metrics(self, df: pd.DataFrame, output_filename: Optional[str] = None) -> str:
        ...
        with pd.ExcelWriter(output_file_path, engine='openpyxl') as writer:
            # Save main results - only the columns we want
        ...
        # Print column list to confirm
        print(f"  - Columns in Results sheet: {df.columns.tolist()}")
